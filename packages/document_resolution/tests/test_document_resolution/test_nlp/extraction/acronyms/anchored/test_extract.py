...
class TestBuildLocalWindowUnit:
    def test_middle_of_text_respects_windows(self, fo):
        text = "0123456789ABCDEFGHIJ"
        fo = fo("X", s=10, e=11)  # points at "A"
        left, right, seg = mod._build_local_window(text, fo, window_left=3, window_right=4)
        assert left == 7
        assert right == 15
        assert seg == text[7:15]

    def test_left_clamps_to_zero(self, fo):
        text = "abcdef"
        fo = fo("X", s=1, e=2)
    ...
    def test_right_clamps_to_len(self, fo):
        text = "abcdef"
        fo = fo("X", s=4, e=5)
    ...
    def test_both_sides_clamp(self, fo):
        text = "abcdef"
        fo = fo("X", s=0, e=6)
    ...
    def test_window_can_be_zero_length(self, fo):
        text = "abcdef"
        fo = fo("X", s=2, e=4)  # "cd"
    ...
    def test_returns_segment_consistent_with_indices(self, fo):
        text = "hello world"
        fo = fo("X", s=6, e=11)  # "world"
...
class TestFoOccurrencePositionUnit:
    def test_returns_relative_offsets(self, fo):
        fo = fo("SSO", s=10, e=13)
        assert mod._fo_occurrence_position(fo, left=7) == (3, 6)

    def test_zero_left_is_identity(self, fo):
        fo = fo("PDF", s=2, e=5)
        assert mod._fo_occurrence_position(fo, left=0) == (2, 5)

    def test_left_equal_start_gives_zero_start(self, fo):
        fo = fo("GPU", s=20, e=23)
