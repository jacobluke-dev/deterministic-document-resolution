...
@pytest.fixture
def extraction_cfg() -> ExtractionConfig:
    return ExtractionConfig()


class TestCompileAnchoredExactContracts:
    def test_returns_non_empty_and_has_expected_kinds(self, extraction_cfg):
        specs = compile_anchored_for_surface("PDF", extraction_cfg)
    ...
    def test_forward_paren_captures_groups(self, extraction_cfg):
        specs = compile_anchored_for_surface("PTO", extraction_cfg)
    ...
    def test_reverse_paren_captures_groups(self, extraction_cfg):
        specs = compile_anchored_for_surface("SSO", extraction_cfg)
    ...
    def test_brackets_variants_work(self, extraction_cfg):
        specs = compile_anchored_for_surface("GPU", extraction_cfg)

        # Forward bracket
        text1 = "Graphics Processing Unit [GPU]"
        _, m1 = _first_match(specs, text1)
        ...
        _, m2 = _first_match(specs, text2)
        assert m2 is not None
        assert m2.group("acr") == "GPU"
        assert m2.group("def") == "Graphics Processing Unit"

    def test_optional_trailing_dot_is_outside_acr_group(self, extraction_cfg):
        specs = compile_anchored_for_surface("U.S", extraction_cfg)
    ...
    def test_optional_quotes_do_not_enter_acr_group(self, extraction_cfg):
        specs = compile_anchored_for_surface("NHS", extraction_cfg)
    ...
    def test_tail_inside_wrapper_does_not_pollute_acr_group(self, extraction_cfg):
        specs = compile_anchored_for_surface("PPE", extraction_cfg)
    ...
    def test_possessive_surface_allowed_but_acr_group_remains_plain(self, extraction_cfg):
        specs = compile_anchored_for_surface("PDF", extraction_cfg)
    ...
    def test_inline_after_resolves_definition_span(self, extraction_cfg):
        specs = compile_anchored_for_surface("NLP", extraction_cfg)
        ...
        a0, a1 = m.span("acr")
        span = resolve_def_span(spec.strategy, seg=seg, m=m, acr_key="NLP", a1_local=a1, cfg=extraction_cfg)
    ...
    def test_inline_before_matches_one_cue(self, extraction_cfg):
        specs = compile_anchored_for_surface("NLP", extraction_cfg)
