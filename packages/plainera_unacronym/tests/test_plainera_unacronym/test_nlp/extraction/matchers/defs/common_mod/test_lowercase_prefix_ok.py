from plainera_unacronym.nlp.extraction.acronyms.matchers.defs.common import _lowercase_prefix_ok
...
class TestLowercasePrefixOkUnit:
    def test_disabled_flag_returns_false(self, _patch):
        ...
        assert (
            _lowercase_prefix_ok(acr="iOS", tokens=["iPhone"], token_idx=0, acr_pos=0, lowercase_prefix_exception=False)
            is False
        )
    ...
    def test_only_applies_at_acr_pos0_and_token0(self, _patch):
        ...
        assert (
            _lowercase_prefix_ok(acr="iOS", tokens=["iPhone"], token_idx=1, acr_pos=0, lowercase_prefix_exception=True)
            is False
        )
        assert (
            _lowercase_prefix_ok(acr="iOS", tokens=["iPhone"], token_idx=0, acr_pos=2, lowercase_prefix_exception=True)
            is False
        )
    ...
    def test_requires_mixed_case_acronym(self, _patch):
        ...
        assert (
            _lowercase_prefix_ok(acr="IOS", tokens=["iPhone"], token_idx=0, acr_pos=0, lowercase_prefix_exception=True)
            is False
        )
    ...
    def test_rejects_allcaps_alpha_token0(self, _patch):
        _patch(_lowercase_prefix_ok, PUNCT_TRIM="", is_mixed_case_acronym=lambda acr: True)
        assert (
            _lowercase_prefix_ok(acr="iOS", tokens=["HTTP"], token_idx=0, acr_pos=0, lowercase_prefix_exception=True)
            is False
        )
