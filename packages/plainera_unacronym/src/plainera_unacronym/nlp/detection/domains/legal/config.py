...
LEGAL_QUOTED_MEANS_RE = re.compile(
    r'"[A-Z][^"\n]{1,80}"\s+(?:shall\s+)?means?\b',
    re.IGNORECASE,
)
...
LEGAL_CONTRACT_NOUNS_RE = re.compile(
    r"\b(?:agreement|services|party|parties|term|obligation|obligations|commence)\b",
    re.IGNORECASE,
)

LEGAL_CAPITALISED_TERM_SHALL_RE = re.compile(
    r"\b(?:The\s+)?"
    r"(?:Agreement|Services|Party|Parties|Customer|Supplier|Company|Contractor|Client|Licensee|Licensor)"
    r"\s+shall\s+\w+",
)

LEGAL_CONTRACT_SUBJECT_SHALL_RE = re.compile(
    r"\b(?:The\s+)?"
    r"(?:Agreement|Services|Party|Parties|Customer|Supplier|Company|Contractor|Client|Licensee|Licensor)"
    r"\s+shall\s+\w+"
)
...
LEGAL_STRONG_CUES = (
    ...
    ("capitalised_term_shall", LEGAL_CAPITALISED_TERM_SHALL_RE, 6),
    ("contract_subject_shall", LEGAL_CONTRACT_SUBJECT_SHALL_RE, 6),
)
...
LEGAL_SUPPORT_CUES = (
    ...
    ("contract_nouns", LEGAL_CONTRACT_NOUNS_RE, 1),
)
