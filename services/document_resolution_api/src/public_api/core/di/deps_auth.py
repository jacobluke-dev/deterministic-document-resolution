...
async def require_api_key(
    ...
    if settings.AUTH_DISABLED and settings.APP_ENV == "local":
        message_logger("AUTH_DISABLED=true in local; skipping API key auth.")
    ...
    if settings.API_KEY_LAST_USED_ASYNC:
        async def _touch() -> None:
            try:
                ...
                message = f"failed to update last_used_at for key_id={parsed.key_id}: {e}"
                message_logger(
                    message=message,
                    level=LogLevel.INFO,
                    logger_type="di.deps.auth",
                    db_sink=sink,
                )
